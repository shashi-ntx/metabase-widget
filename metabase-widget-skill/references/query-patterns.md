...
| `NUSIGHTS_EVENTS_ACTIVITYTYPE_DEFAULT_HISTORICAL_TBL_FLAT` | Flattened view that pulls JSON keys into regular columns   |

For Metabase questions, the reliable choice is the `_FLAT` view because its columns can be filtered directly in the UI. Always use the full name. The shorthand `NUSIGHTS_EVENTS_FLAT` does not exist as a Snowflake object.

### Important field mapping

The full mapping, including Query Builder display names, is in `data-model.md`. The core columns are:

| Telemetry event field | DB column (SQL)    | Query Builder display name |
| --------------------- | ------------------ | -------------------------- |
| `actionName`          | `DESTINATION_NAME` | `Destination Name`         |
| `actionType`          | `ACTION_TYPE`      | `Action Type`              |
| `featName`            | `FEATNAME`         | `Featname`                 |
| `pageSection`         | `PAGE_SECTION`     | `Page Section`             |
| `subPageSection`      | `SUB_PAGE_SECTION` | `Sub Page Section`         |
...
| Bucketing an aggregate (e.g. "clusters with 1, 2–10, 11–30 VGs") | Query Builder with "Summarize again" if simple; otherwise SQL |
| Funnel steps with different filters                              | SQL                                                           |
| Time between two event types                                     | SQL                                                           |
| Average / median time across matched events                      | SQL                                                           |

Prefer the Query Builder whenever it can express the question. Designers and PMs can tweak those questions themselves, and in Stage 2 the skill saves them as Query Builder questions (see `api-integration.md`).
...
1. Pick **Nusights Events Activitytype Default Historical Tbl Flat**
...
1. Filter on the shared context, such as `Page Section`
...
Metabase cannot use the visual builder to filter deeply inside Snowflake `VARIANT` / JSON columns. That is why the `NUSIGHTS_EVENTS_ACTIVITYTYPE_DEFAULT_HISTORICAL_TBL_FLAT` view matters so much: it turns payload keys into normal columns.
